...
def main():
    ...
    print(
        ...
        f"{listing}\n\n"
        "Rule: the default is NO comment. Keep one only where the code would look "
        "wrong, arbitrary or accidental without it — a hidden constraint, a "
        "workaround, a surprising consequence — and then only one or two lines. "
        "Never restate the line below it; never write a doc comment that only "
        "expands the identifier's name.\n\n"
